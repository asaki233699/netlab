...
void tcp_out(tcp_conn_t *tcp_conn, buf_t *buf, uint16_t src_port, uint8_t *dst_ip, uint16_t dst_port, uint8_t flags) {
    ...
    uint16_t new_checksum = transport_checksum(NET_PROTOCOL_TCP, buf, net_if_ip, dst_ip);
    hdr->checksum16 = swap16(new_checksum);
...
void tcp_in(buf_t *buf, uint8_t *src_ip) {
    ...
    hdr->checksum16 = 0;
    if (transport_checksum(NET_PROTOCOL_TCP, buf, src_ip, net_if_ip) != swap16(checksum))
        return;
    ...
    // TODO
    size_t payload_len = buf->len - tcp_hdr_sz;
    if (payload_len > 0) {
        tcp_handler_t *handler = map_get(&tcp_handler_table, &host_port);
        if (handler && *handler) {
            (*handler)(tcp_conn, buf->data + tcp_hdr_sz, payload_len, remote_ip, remote_port);
        }else{
            buf_add_header(buf, -((int)tcp_hdr_sz));
            // 未注册处理程序，发送 ICMP 端口不可达
